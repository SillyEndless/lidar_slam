...
public:
    ...
    //把一帧激光数据转换为点云坐标，用eigen_pts来存储点云坐标
                                                   ...
                                                   std::vector<Eigen::Vector2d>& eigen_pts)
    {
        ...
        for(int i = 0; i < msg->ranges.size();i++)
        {
            ...
            Eigen::Vector3d opt = Tmatrix * lpt;//激光点在里程计坐标系上的坐标
    ...
    void championLaserScanCallback(const champion_nav_msgs::ChampionNavLaserScanConstPtr& msg)
    {
        ...
        //调用imls进行icp匹配，并输出结果．
        m_imlsMatcher.setSourcePointCloud(nowPts);//当前点云帧
        m_imlsMatcher.setTargetPointCloud(m_prevPointCloud);//上一帧点云
