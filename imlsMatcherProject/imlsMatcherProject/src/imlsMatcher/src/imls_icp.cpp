                                         ...
                                         double& height)
{
    ...
    //根据函数进行投影．
    //求空间中点到曲面的最近距离
    //nearPoints是点x的近点点云集合。用kd-tree求得。
    //nearNormals是nearPoints中点对应的法向量集合

    double Wi=0,Zi=0;
    for(int i=0;i<nearPoints.size();i++) {
        Eigen::Vector2d tempVi =x - nearPoints[i];
         Wi = std::exp(-pow(tempVi.norm(),2)/ (m_h * m_h));
        //double Wi = exp(-tempVi.squaredNorm()/ (m_h * m_h));
         Zi=Wi*tempVi.transpose()*nearNormals[i];
...
Eigen::Vector2d IMLSICPMatcher::ComputeNormal(std::vector<Eigen::Vector2d> &nearPoints)
{
    ...
    Eigen::Vector2d sumVector(0.0,0.0) ;
    Eigen::Matrix2d sum;
    sum.setZero();
    for(int i=0;i<nearPoints.size();i++){
        sumVector+=nearPoints[i];
    }
    Eigen::Vector2d meanVector=sumVector/nearPoints.size();//均值

    for(int i=0;i<nearPoints.size();i++){
        Eigen::Vector2d dVector=nearPoints[i]-meanVector;
        Eigen::Matrix2d temp=dVector*dVector.transpose();
        sum+=temp;
    }
    Eigen::Matrix2d variance=sum/(nearPoints.size()-1);//协方差矩阵

    Eigen::EigenSolver<Eigen::Matrix2d> es(variance);
    Eigen::MatrixXf::Index evalsMin;
    es.eigenvalues().real().rowwise().sum().minCoeff(&evalsMin);//得到最小特征值的位置
    normal<<es.eigenvectors().real()(0,evalsMin),es.eigenvectors().real()(1,evalsMin);//得到对应的法向量


    //确定lamda和normal是否对应
    //    double lamda=es.eigenvalues().real()[evalsMin];
    //    Eigen::Vector2d v3=(variance-lamda*Eigen::Matrix2d::Identity())*normal;
