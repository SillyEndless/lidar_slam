...
public:
    ...
    void rosLaserScanCallback(const sensor_msgs::LaserScanConstPtr& msg)
    {
        ...
        //生成地图.利用上一帧的数据构建占据栅格地图。
        map_t* map = CreateMapFromLaserPoints(m_prevLaserPose,m_prevPts,0.25);
        //m_prevLaserPose为上一帧激光的第一束激光在里程计坐标系下的位姿。
        // 这个位姿不是由里程计读取出来的，而是由激光计算出来的。
        //m_prevPts为上一帧中所有激光点在激光参考系下的坐标。
        // 生成局部地图。局部地图原点为上一帧第一束激光点的位置。地图分辨率为0.25.
        //栅格地图，激光点视为障碍物，每个栅格存储有到最近障碍物的距离，由距离计算出分数。
        //这样可得到每个栅格的分数(势场)。


        //进行优化．
        //初始解为上一帧激光位姿+运动增量。激光位姿：激光发射器在里程计坐标系下的位姿。
        Eigen::Vector3d deltaPose = nowPose - m_odomPath.back();
        deltaPose(2) = GN_NormalizationAngle(deltaPose(2));//odom坐标系下的增量。
        ...
        R_odom << cos(theta), -sin(theta), 0, 
                  sin(theta),  cos(theta), 0,
                       0,          0,      1;
        Eigen::Vector3d finalPose = m_prevLaserPose + R_laser * R_odom.transpose() * deltaPose;//计算出来的激光发射器的里程计位姿。
        finalPose(2) = GN_NormalizationAngle(finalPose(2));

        GaussianNewtonOptimization(map,finalPose,nowPts);
        //map是由上一帧激光数据构建的地图。finalPose是初始解.利用该初始值，不断迭代更新finalPose.

        //更新数据．
        m_prevLaserPose = finalPose;
        m_prevPts = nowPts;

        std::cout <<"Init Pose:"<<nowPose.transpose()<<std::endl;
